...
trait ListsSolutions {
  ...
  def isPalindrome[T](list: List[T]): Boolean = list.take(list.length/2) == list.takeRight(list.length/2).reverse
  ...
  def encodeModified[T](list: List[T]): List[Any] = pack(list) map { ls => 
    if (ls.length > 1) (ls.length, ls.head)
    else ls.head
  }
  ...
  def duplicateN[T](n: Int, list: List[T]): List[T] = list flatMap { List().padTo(n, _) }
  ...
  def groups[T](ns: List[Int], list: List[T]): List[List[List[T]]] = {
    def addGroupN(gs: List[List[List[T]]], n: Int): List[List[List[T]]] = {
      gs.flatMap(xss => {
        combinations(n, list diff xss.flatten).map(xs => xss :+ xs)
      })
    }
    
    def groupsIter(ms: List[Int], gs: List[List[List[T]]]): List[List[List[T]]] = {
      if (ms.isEmpty) gs
      else groupsIter(ms.tail, addGroupN(gs, ms.head))
    }
    
    ns match {
      case Nil => List(List(Nil))
      case _ => groupsIter(ns, List(List()))
    }
  }
  ...
  def lsortFreq[T](list: List[List[T]]): List[List[T]] = {
    list.sortWith(
        (xs,ys) => list.count(_.length == xs.length) < list.count(_.length == ys.length)
        )
  }
}
